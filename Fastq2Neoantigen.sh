...
if [ $normal_fq2 != "None" ];then
    ...
    ${fasp} --thread 10  -i ${normal_fq1}  -o ${outpath}/${normalname}/01.QC/${normalname}_R1.fastq.gz  --unpaired1 ${outpath}/${normalname}/01.QC/${normalname}_R1.unpaired.fastq.gz  --html ${outpath}/${normalname}/01.QC/${normalname}.html --json ${outpath}/${normalname}/01.QC/${normalname}.json
fi
...
if [ $xomic = "RNA" ];then
    ...
    ${sambamba} view -t 20 --sam-input --format=bam ${outpath}/${tumorname}/02.Aln/${tumorname}.BWA.aligned.sam > ${outpath}/${tumorname}/02.Aln/${tumorname}.BWA.aligned.bam
    ${sambamba} sort -t 20 ${outpath}/${tumorname}/02.Aln/${tumorname}.BWA.aligned.bam > ${outpath}/${tumorname}/02.Aln/${tumorname}.BWA.aligned.sorted.bam
    ${sambamba} index -t 20 ${outpath}/${tumorname}/02.Aln/${tumorname}aligned.sorted.bam  > ${outpath}/${tumorname}/02.Aln/${tumorname}.BWA.aligned.sorted.bam.bai
    
    ${sambamba} view -t 20 --sam-input --format=bam ${outpath}/${normalname}/02.Aln/${normalname}.BWA.aligned.sam > ${outpath}/${normalname}/02.Aln/${normalname}.BWA.aligned.bam
    ${sambamba} sort -t 20 ${outpath}/${normalname}/02.Aln/${normalname}.BWA.aligned.bam > ${outpath}/${normalname}/02.Aln/${normalname}.BWA.aligned.sorted.bam
...
if [ $xomic = "RNA" ];then
    ...
    /usr/bin/java -jar -Xmx20g ${VarDict} -G ${geneome} -f 0.002 -N ${tumorname} -b "${outpath}/${tumorname}/02.Aln/${tumorname}.BWA.aligned.sorted.bam|${outpath}/${normalname}/02.Aln/${normalname}.BWA.aligned.sorted.bam"  -c 1 -S 2 -E 3 -g 4 ${bedfile} -th 40 | ${testsomaticR} | ${var2vcf_pairedpl} -N "${tumorname}|${normalname}"  -f 0.002 >${outpath}/${tumorname}/03.SNV/${tumorname}.vardict.vcf
fi
...
docker run --rm -v ${vepCacheDir}:/opt/vep/cache -v ${outpath}/${tumorname}/03.SNV:/temp/03.SNV:Z  -v ${vepPlugins}:/opt/vepPlugins --user ${userid}:${groupid} ensemblorg/ensembl-vep  vep  --fork 20 --input_file  /temp/03.SNV/${tumorname}.vardict.vcf --output_file  /temp/03.SNV/${tumorname}.vardict.vep.vcf --format vcf --vcf --symbol --species mus_musculus  --terms SO --tsl --offline --cache  --dir_cache /opt/vep/cache  --plugin Frameshift --plugin Wildtype  --dir_plugins /opt/vepPlugins
